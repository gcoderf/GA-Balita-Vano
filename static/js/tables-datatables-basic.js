...
// datatable (jquery)
$(function () {
  var dt_basic_table = $('.datatables-makanan'),
    dt_basic_table_akg = $('.datatables-akg'),
    dt_complex_header_table = $('.dt-complex-header'),
    dt_row_grouping_table = $('.dt-row-grouping'),
    dt_multilingual_table = $('.dt-multilingual'),
    dt_basic;

  // DataTable Bahan Pangan
  // --------------------------------------------------------------------
  ...
  if (dt_basic_table_akg.length) {
    dt_basic = dt_basic_table_akg.DataTable({
      ...
      columns: [{ data: '' }, { data: 'kelompok umur' }, { data: 'kelompok umur' }, { data: 'kelompok umur' }, { data: 'energi' }, { data: '' }],
      columnDefs: [
        {
          // For Responsive
          className: 'control',
        ...
        },
        {
          ...
          targets: -2,
          render: function (data, type, full, meta) {
            var $status_number = full['kelompok umur'];
            var $status = {
              raw: { title: 'Mentah', class: 'bg-label-primary' },
              processed: { title: 'Olahan', class: ' bg-label-success' },
              3: { title: 'Rejected', class: ' bg-label-danger' },
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['kelompok umur'];
            },
          }),
          type: 'column',
          renderer: function (api, rowIdx, columns) {
